...
def test_calc_amplitude_UK():
    ...
    for (local_mag, hypo_dist) in zip([-3, 0.0, 1.0, 2.0, 3.0, 4.0], [0.1, 10, 20, 30, 100]):
        expect = np.power(10, (local_mag - a*np.log10(hypo_dist) - b*hypo_dist - c - d*np.exp(e*hypo_dist)))
        actual = calc_ampl_from_magnitude(local_mag, hypo_dist, region='UK')
        assert np.isclose(actual, expect), f"Failed for local_mag={local_mag}, hypo_dist={hypo_dist}"
...
@pytest.mark.parametrize('noise, distance, snr, mag_delta, mag_min', [
    (10.0, 1.0, 1, 0.1, -2.0),
    (0.5, 2.0, 2, 0.5, -4),
    (0.01, 100, 1, 0.2, 0),
    (1, 0.01, 10, 0.05, -5)])
def test_est_min_ML_at_station(noise, distance, snr, mag_delta, mag_min):
    ...
    result = _est_min_ML_at_station(noise,
                                    mag_min,
                                    mag_delta,
                                    distance,
                                    snr,
    ...
    assert np.isclose(expected - result, 0, atol=mag_delta), f"Expected {expected}, got {result}, mag_delta {mag_delta}"
    ...
    result = _est_min_ML_at_station(noise, mag_min, mag_delta, distance, snr, method='ML', region='CAL')
    assert np.isclose(expected - result, 0, atol=mag_delta), f"Failed for CAL region: expected {expected}, got {result}"
