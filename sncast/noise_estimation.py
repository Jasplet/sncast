...
def estimate_noise_displacement(station_ppsd, f0=5, n=0.5, case='worst', verbose=False):
    ...
    displacement = psd_db_to_displacement_amplitude(mean_psd_db,
                                                    f0=f0,
                                                    f1=f1,
                                                    f2=f2)
...
def estimate_noise_velocity(station_ppsd, f0=5, n=0.5, case='worst', verbose=False):
    ...
    velocity = psd_db_to_velocity(mean_psd_db,
                                  f0=f0,
                                  f1=f1,
                                  f2=f2)
...
def make_noise_estimate_for_ppsds(Inventory, case, kind='displ', **kwargs):
    ...
    for Network in Inventory:
        for Station in Network:
            path = Path(f'/Users/eart0593/Projects/NEP_consulting/UK_network/data/ppsds/1year_span/{Network.code}/')
            ...
            try:
                ...
                if kind == 'displ':
                    if 'f0' in kwargs:
                        displ_m = estimate_noise_displacement(ppsd,
                                                              case=case,
                                                              f0=kwargs['f0'])
                    else:
                    ...
                    if 'f0' in kwargs:
                        vel_ms = estimate_noise_velocity(ppsd,
                                                         case=case,
                                                         f0=kwargs['f0'])
                    else:
